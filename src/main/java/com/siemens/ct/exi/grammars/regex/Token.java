...
class Token implements java.io.Serializable {
	...
	static {
		...
		Token.token_not_wordchars = Token
	...
	final int getMinLength() {
		...
		default:
			throw new RuntimeException("Token#getMinLength(): Invalid Type: "
					+ this.type);
		}
	}
	...
	final int getMaxLength() {
		...
		default:
			throw new RuntimeException("Token#getMaxLength(): Invalid Type: "
					+ this.type);
		}
	}
	...
	final int analyzeFirstCharacter(RangeToken result, int options) {
		...
		case STRING:
			...
			if (REUtil.isHighSurrogate(cha)
					...
					&& REUtil
							.isLowSurrogate((ch2 = this.getString().charAt(1))))
	...
	final void findFixedString(FixedStringContainer container, int options) {
		...
		case CONCAT:
			...
			for (int i = 0; i < this.size(); i++) {
				...
				if (prevToken == null
						|| prevToken.isShorterThan(container.token)) {
	...
	static protected RangeToken getRange(String name, boolean positive) {
		if (Token.categories.size() == 0) {
			synchronized (Token.categories) {
				...
				Token.categories2.put("IsAlpha",
						Token.complementRanges(isalpha));
				...
				Token.categories2.put("IsAlnum",
						Token.complementRanges(isalnum));
				...
				Token.categories2.put("IsSpace",
						Token.complementRanges(isspace));
				...
				Token.categories2.put("IsASCII",
						Token.complementRanges(isascii));
				Token.registerNonXS("IsASCII");

				Token isnotgraph = Token.createRange();
				isnotgraph.mergeRanges(ranges[CHAR_OTHER]);
				isnotgraph.addRange(' ', ' ');
				Token.categories.put("IsGraph",
						Token.complementRanges(isnotgraph));
				...
				isxdigit.addRange('a', 'f');
				Token.categories.put("IsXDigit",
						Token.complementRanges(isxdigit));
			...
			case CHAR:
				...
				default:
					if (this.chardata >= 0x10000) {
						...
						ret = "\\v"
								+ pre.substring(pre.length() - 6, pre.length());
					} else
						ret = "" + (char) this.chardata;
				}
				break;
	...
	static class UnionToken extends Token implements java.io.Serializable {
		...
		void addChild(Token tok) {
			...
			StringBuffer buffer;
			int nextMaxLength = (tok.type == CHAR ? 2 : tok.getString()
					.length());
			if (previous.type == CHAR) { // Replace previous token by STRING
				...
				buffer = new StringBuffer(previous.getString().length()
						+ nextMaxLength);
		...
		public String toString(int options) {
			...
			if (this.type == CONCAT) {
				if (this.children.size() == 2) {
					...
					} else if (ch2.type == NONGREEDYCLOSURE
							&& ch2.getChild(0) == ch) {
						ret = ch.toString(options) + "+?";
					} else
					...
					for (int i = 0; i < this.children.size(); i++) {
						sb.append(((Token) this.children.elementAt(i))
								.toString(options));
					}
					ret = new String(sb);
				}
			...
			if (this.children.size() == 2 && this.getChild(1).type == EMPTY) {
				...
				StringBuffer sb = new StringBuffer();
				sb.append(((Token) this.children.elementAt(0))
						.toString(options));

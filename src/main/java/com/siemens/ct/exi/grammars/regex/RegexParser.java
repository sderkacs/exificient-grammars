	...
	public void setLocale(Locale locale) {
		try {
			...
			throw new RuntimeException(
					"Installation Problem???  Couldn't load messages: "
							+ mre.getMessage());
		}
	}
...
/**
	...
	Token processNegativelookahead() throws ParseException {
		this.next();
		Token tok = Token
				.createLook(Token.NEGATIVELOOKAHEAD, this.parseRegex());
		if (this.read() != T_RPAREN)
			throw ex("parser.factor.1", this.offset - 1);
		this.next(); // ')'
		return tok;
	...
	Token processNegativelookbehind() throws ParseException {
		this.next();
		Token tok = Token.createLook(Token.NEGATIVELOOKBEHIND,
				this.parseRegex());
		if (this.read() != T_RPAREN)
			throw ex("parser.factor.1", this.offset - 1);
		this.next(); // ')'
	...
	protected RangeToken parseCharacterClass(boolean useNrange)
		...
		while ((type = this.read()) != T_EOF) {
			...
			if (type == T_BACKSOLIDUS) {
				...
				if (nameend + 1 >= this.regexlen
						|| this.regex.charAt(nameend + 1) != ']')

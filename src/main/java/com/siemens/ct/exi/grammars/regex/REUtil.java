...
public final class REUtil {
		...
		for (int i = 0; i < opts.length(); i++) {
			...
			if (v == 0)
				throw new ParseException(
						"Unknown Option: " + opts.substring(i), -1);
			options |= v;
		}
		return options;
	}
	...
	public static void main(String[] argv) {
		...
		try {
			...
			if (argv.length == 0) {
				System.out
						.println("Error:Usage: java REUtil -i|-m|-s|-u|-w|-X regularExpression String");
				System.exit(0);
			}
			...
			for (int i = 0; i < match.getNumberOfGroups(); i++) {
				...
				if (match.getBeginning(i) < 0)
					...
					System.out.print(match.getBeginning(i) + ", "
							+ match.getEnd(i) + ", ");
	...
	public static RegularExpression createRegex(String pattern, String options)
		...
		synchronized (REUtil.regexCache) {
			...
			if (re != null) {
				if (i != 0) {
					System.arraycopy(REUtil.regexCache, 0, REUtil.regexCache,
							1, i);
